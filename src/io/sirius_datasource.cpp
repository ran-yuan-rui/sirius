...
/// Telemetry-carrying variant: the callback owns a shared_ptr to the sink so
/// emission never depends on another object's lifetime (async contract), and
/// classifies cancellation structurally from the settled exception.
std::future<size_t> bridge_semi_to_std(exec::semi_future<size_t>&& sf,
