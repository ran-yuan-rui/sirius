...
list_objects_v2_page parse_list_objects_v2(std::string_view xml)
{
  ...
  // Object keys: the <Key> inside each <Contents> block. Scoping to <Contents>
  // (rather than scanning every <Key> in the document) keeps a stray <Key> that
  // an S3-compatible service might place elsewhere — or a future response
  // extension — from being mistaken for an object key. <CommonPrefixes> rollups
  // use <Prefix>, so they are excluded for free.
  ...
  for (std::size_t pos = 0;;) {
    ...
    if (auto const key = element_text(block, "Key"); key.has_value()) {
      page.keys.push_back(xml_unescape(*key));
    }
