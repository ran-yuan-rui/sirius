      ...
      std::make_unique<exec::scoped_dispatcher>(_thread_pool, _config.thread_pool.num_threads))
{
  ...
  if (_config.use_sirius_datasource) {
    uring_ctx = std::make_shared<sirius::io::uring_ioctx>(_config.uring_ring_entries,
  ...
  if (_config.enable_prefetch_cache && !_io_ctxs.empty()) {
    ...
    // Share the buffer_pool across every registered ioctx — prefetching_cache
    // claims thread safety, so concurrent reads through uring and S3 can
    // share cache slots keyed by raw_file_cache_id() (namespace-prefixed,
    // no cross-backend collision).
