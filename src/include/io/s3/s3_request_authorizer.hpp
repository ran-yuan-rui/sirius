...
class s3_request_authorizer {
 public:
   ...
   *                          `&`-joined, and **sorted by key** (SigV4 canonical
   *                          order), WITHOUT any auth params — e.g.
   *                          @c "list-type=2&max-keys=1000&prefix=a%2Fb" (with
   *                          @c "continuation-token=..." sorted in first). The
   *                          header-signing path signs this string verbatim, so an
   *                          unsorted query would be signed but rejected by S3;
   *                          the presigned path re-sorts when merging the
   *                          @c X-Amz-* params, but callers should pass sorted
   *                          regardless. The authorizer adds auth (presigned
   *                          @c X-Amz-* in the URL query, or @c Authorization
   *                          headers) and returns the URL to GET plus headers to
   *                          attach verbatim.
   ...
   */
  [[nodiscard]] virtual s3_authorized_request authorize_list(std::string const& /*bucket*/,
