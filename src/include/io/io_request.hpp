...
class request_manager {
 public:
  ...
  /// Attach a telemetry sink + read identity (only when a sink is active).
  /// Must happen before the request's chunks are enqueued; the backend record
  /// is emitted exactly once from the destructor, before the future settles.
  /// All telemetry state lives behind this single allocation so an untracked
  /// request pays exactly one null pointer.
  ...
  /// One actually-scheduled re-attempt (transient or auth). Counted at
  /// schedule time so failed requests report their retries too.
...
struct rx_request_t {
  ...
  /// Attach a telemetry sink + read identity to the shared manager (all chunks
  /// of one caller request share it). No-op for empty requests.
  void install_telemetry(std::shared_ptr<io_telemetry_sink> sink, const io_read_context& ctx)
  {
