...
class templated_ioctx : public sirius_ioctx {
 public:
  ...
  /// Attach the ioctx's sink + the caller's read identity to the request's
  /// shared manager (all chunks of one request share it). No-op when either
  /// side is absent — the off path stays a single branch.
