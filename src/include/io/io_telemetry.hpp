...
/**
 * Backend-neutral IO telemetry seam.
 *
 * This header must stay free of any telemetry-backend dependency (no
 * generated bridge headers, no Rust types): consumers of the records decide
 * what to do with them behind @c io_telemetry_sink. With no sink registered
 * (the default) every emission point in the IO layer is a single untaken
 * branch — no id minting, no clock reads, no attribution copies.
 */
...
/// Immutable attribution stamped once per scan split (and per call site for
/// non-scan phases); flows BY VALUE with each read into the io layer. Never
/// shared by pointer into asynchronous paths — the prefetch cache outlives
/// the datasource that armed it.
 ...
 * caller threads, reactor worker threads, and prefetch-cache threads, and
 * may not block or throw. Production sinks must be non-blocking; a test
 * recorder may serialize internally.
 */
...
/// Per-read identity carried BY VALUE (or by pointer valid for the duration
/// of a synchronous call) from the datasource leaf into the request layer so
/// backend records join their logical record. Built only when a sink is
/// active.
...
/// Process-unique read id: random high half (minted once), counter low half.
/// Deliberately NOT a real UUID — this header must not depend on the
/// engine's generated UUID type, and joins are process-local.
namespace detail {
/// Process entropy with a no-throw fallback (std::random_device may throw on
/// exotic platforms; fall back to clock/address entropy rather than
/// terminating from a noexcept context).
...
/// Session-scoped salted 64-bit object identity (dictionary-resistant within
/// the salt's lifetime; the salt is minted once per process and never
/// recorded). Centralized so every emitter derives identity identically.
