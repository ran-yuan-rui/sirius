...
struct scan_manager_config {
  ...
  /// Optional IO telemetry sink (io/io_telemetry.hpp), propagated to every
  /// backend's reactor context and ioctx at construction. Runtime-injected
  /// (not file-settable); null (the default) keeps every emission point in
  /// the IO layer structurally inert.
